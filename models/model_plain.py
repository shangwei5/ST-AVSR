...
class ModelPlain(ModelBase):
    ...
    def feed_data(self, data, need_H=True):
        ...
        self.H, self.L, self.H_flows, self.dis_encodings, self.is_b2t, self.out_path, self.input_path = data  # rs_imgs, gs_imgs, fl_imgs, prior_imgs, is_b2t, out_paths, input_path
    ...
    def netG_forward(self):
        ...
        self.E = self.netG(self.L, self.dis_encodings, self.is_b2t)
        # b, c, h, w = self.E[0].shape  # (8, 3*ext_frames, 256, 256)
        # self.num_gs = int(c / 3)
        if ori_h % 32 != 0 or ori_w % 32 != 0:
            self.E = self.E[:, :, :ori_h, :ori_w]
            # self.dis_encodings
        b, num_gs, c, h, w = self.H.shape  # (8, 1, 3, 256, 256)
        self.H = self.H.reshape(b, num_gs * c, h, w).detach()  # (8, 3*1, 256, 256)

        b, n, c, h, w = self.H_flows.shape  # (8, 2*ext_frames, 2, 256, 256)
        self.H_flows = self.H_flows.reshape(b, n * c, h, w)  # (8, 4*ext_frames, 256, 256)
        self.H_flows = (F.interpolate(self.H_flows, scale_factor=0.5, mode="bilinear", align_corners=False) * 0.5).detach()
    # ----------------------------------------
