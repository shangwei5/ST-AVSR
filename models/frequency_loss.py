...
class FrequencyLoss(nn.Module):
    ...
    def forward(self, pred, target, matrix=None, **kwargs):
        ...
        # print(loss.shape, matrix.shape)
        loss = (loss * (1 - matrix.unsqueeze(1))).sum() / (1 - matrix).sum() / 3 / loss.shape[1]

        return loss

class FrequencyLoss_onlyP(nn.Module):
    """Frequency loss.
    ...
    def __init__(self,
                 ...
                 log_matrix=False,
                 batch_matrix=False):
        super(FrequencyLoss_onlyP, self).__init__()
        self.loss_gamma = loss_gamma
        self.matrix_gamma = matrix_gamma
        self.patch_factor = patch_factor
        self.ave_spectrum = ave_spectrum
    ...
    def loss_formulation(self, recon_freq, real_freq, matrix=None):
        # frequency distance using (squared) Euclidean distance
        tmp = (recon_freq - real_freq) ** 2
        loss = torch.sqrt(tmp) ** self.loss_gamma   #torch.sqrt(tmp[..., 0] + tmp[..., 1] + 1e-12) ** self.loss_gamma
        # if self.with_matrix:
        #     # spectrum weight matrix
        #     if matrix is not None:
        #         # if the matrix is predefined
