...
class ModelBase():
    ...
    def save_lora_network(self, save_dir, network, network_label, iter_label):
        save_filename = '{}_{}.pth'.format(iter_label, network_label)
        save_path = os.path.join(save_dir, save_filename)
        network = self.get_bare_model(network)
        # state_dict = network.state_dict()
        ...
        #     state_dict[key] = param.cpu()
        # torch.save(state_dict, save_path)
        torch.save(lora.lora_state_dict(network), save_path)

    # ----------------------------------------
    # load the state_dict of the network
    # ----------------------------------------
    def load_network(self, load_path, network, strict=True, param_key='params', is_gmflow=False):
        network = self.get_bare_model(network)
        if strict:
            if is_gmflow:
            ...
            state_dict_old = torch.load(load_path) #['state_dict']
            if param_key in state_dict_old.keys():
                state_dict_old = state_dict_old[param_key]
            state_dict = network.state_dict()

            # pretrained_dict = {k: v for k, v in state_dict.items() if
            #                    k in state_dict_old and v.shape == state_dict_old[k].shape}
            # # 更新现有的model_dict
            # model_dict.update(pretrained_dict)

            for key, param in state_dict.items():
                if key not in state_dict_old:
                    print('--------------------------Not in model:', key)
                    continue
            # for ((key_old, param_old),(key, param)) in zip(state_dict_old.items(), state_dict.items()):
                if param.shape == state_dict_old[key].shape:
                    print(key, "loading successfully!")
