...
def get_single_patch(args, patch_size=17, scale=1):
    """
    ...
    ret = args[iy:iy + ip, ix:ix + ip, :]

    return ret

def get_single_scale_patch(args, patch_size=(11,11), scale=1):
    """
    Get patch from an image
    """
    ih, iw, _ = args.shape
    ...
    ix = random.randrange(0, iw - ipy + 1)
    iy = random.randrange(0, ih - ipx + 1)

    ret = args[iy:iy + ipx, ix:ix + ipy, :]
...
def singlenp2Tensor(args, rgb_range=255, n_colors=1):
    def _np2Tensor(img):
...
def single_data_augment(args, hflip=True, rot=True):
...
def calc_grad_sobel(img, device='cuda'):
    ...
    sobel_filter_X = torch.from_numpy(sobel_filter_X).float().to(img.device)
    sobel_filter_Y = torch.from_numpy(sobel_filter_Y).float().to(img.device)
...
def calc_meanFilter(img, kernel_size=11, n_channel=1, device='cuda'):
    ...
    mean_filter_X = torch.from_numpy(mean_filter_X).float().to(img.device)
