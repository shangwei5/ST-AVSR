...
def tensor2uint_list(img):
    ...
    return [_tensor2uint(a) for a in img]

def masktensor2uint(img):
...
def freq_tensor2uint(img, input_freq):
    img = torch.abs(img)
    img = (img - torch.min(torch.abs(input_freq))) / (
            torch.max(torch.abs(input_freq)) - torch.min(torch.abs(input_freq)))
    img = img.data.squeeze().float().clamp_(0, 1).cpu().numpy()
    if img.ndim == 3:
        img = np.transpose(img, (1, 2, 0))
    # img = np.log(abs(img * 255.0))
    return np.uint8((img*255*255.).round())
...
# SSIM
# --------------------------------------------
def calculate_ssim(img1, img2, border=20):
    '''calculate SSIM
